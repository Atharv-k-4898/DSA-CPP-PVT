...
Node * sortedLLIntoBST(Node *&st,int n){
    //base case
    if(n<=0 || st==NULL) return NULL;
    Node *leftSubtree=sortedLLIntoBST(st,n-1-n/2);
    Node *root=st;
    root->left=leftSubtree;//////////////// imp line
    st=st->right;
    //right part solve karna he
    root->right=sortedLLIntoBST(st,n/2);
    return root;
}
int main()
{
    ...
    printLL(st);
    Node *root1 = sortedLLIntoBST(st, 9); 
    cout << "\nBST from Sorted Linked List (Level Order):" << endl;
    levelOrderTraversal(root1);
    return 0;
}
