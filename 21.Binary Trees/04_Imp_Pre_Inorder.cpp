...
Node *constructTree(map<int, int>&valToIndexMap,
                    int preorder[], int inorder[], int &preIdx, int inorderStart, int inorderEnd, int size)
{
   ...
   // int position = searchInOrder(inorder, size, element);
    ...
    root->left = constructTree(valToIndexMap,preorder, inorder, preIdx, inorderStart, position - 1, size);
    root->right = constructTree(valToIndexMap,preorder, inorder, preIdx, position + 1, inorderEnd, size);
...
int main()
{
    ...
    Node *root = constructTree(valToIndexMap, preOrder, inOrder, preIdx, inorderStart, inorderEnd, size);
